...
FRAME_UPDATE_RATE    = 60  

def get_default_board():
    """
    Khởi tạo bàn cờ theo cách sắp xếp cờ vua mặc định.
    Trả về dictionary chứa tên quân và vị trí của chúng trên bàn cờ.
    Tất cả key đều ở dạng lowercase.
    """
    board = {
...
# Cập nhật bảng cờ với quân cờ
def update_board_with_pieces(board, squares, frame):
    """
    Cập nhật quân cờ vào bảng theo vị trí và tên quân.
    Vẽ tên quân (màu đen, font nhỏ) ở góc trên bên trái of mỗi ô.
    """
    for (sx1, sy1, sx2, sy2, name) in squares:
        # Lấy tên quân cờ ở ô đó (key luôn lowercase)
        piece_name = board.get(name.lower(), 'none')
        if piece_name != 'none':
            # Chọn font nhỏ (0.3) và màu đen (0,0,0)
            cv2.putText(frame, piece_name, (sx1 + 2, sy1 + 15),
...
def move_on_chrome(hwnd, squares, src, dst):
    ...
    # Tạo dict map từ ô -> (sx1, sy1, sx2, sy2)
    ...
    if src not in coord_map or dst not in coord_map:
        ...
        return False   # Trả về False nếu move không hợp lệ
    ...
    return True    # Trả về True nếu đã click thành công
...
def main():
    ...
    # Lấy bàn cờ mặc định (toàn bộ key ở dạng lowercase)
    ...
    while running:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_q:
                    ...
                    if len(parts) == 2:
                        ...
                        if detector.squares:
                            success = move_on_chrome(hwnd, detector.squares, src, dst)
                            if success:
                                # Cập nhật board_state
                                piece = board.get(src, 'none')
                                if piece != 'none':
                                    board[dst] = piece
                                    board[src] = 'none'
                            else:
                                print("[Move] Click không thành công.")
                        else:
        ...
        if annotated is not None:
            update_board_with_pieces(board, detector.squares, annotated)
